            ...
            <div className="grid grid-cols-3 gap-2">
                {capitulos.map((norma: any) => (
                    <Link className='w-full' key={norma} href={`/categorias/${tit}/${norma.nombre}`}>
                        <button className="font-sans rounded-full bg-sky-700 px-20 py-2 mt-3 font-sans font-extralight w-full uppercase text-white" style={{'height': '90px'}}><h3 className='text-2xl'>{norma.nombre}</h3><p className='text-xxs'>{norma.descripcion}</p></button>
                    </Link>
                ))}
            </div>
        </main>
